...
We'll examine first how to check password submitted by the users of your application and then how to generate
random passwords.

== Validating Passwords with PasswordChecker

Let's assume we need the users of our application to create a password before they can log in and do some
work. Let's also assume that we would like this password to be minimally secure, and therefore we'll require
that this password be at least 8 characters long and contain at least one lower-case letter, one upper-case letter and
one digit.

We can create a `PasswordChecker` to do just that:
...
that either indicates that the password is `OK` or the first error condition encountered, so that it might be
...
class, which contains methods to identify the character list concerned and the error in the count.
...
As you can see there are 4 problems with this password that can all get reported with appropriate details:

* it's too short (error 0);
* the character `$` is not allowed (error 1);
* the character `@` is not allowed (error 2);
* the password doesn't contain any digit (error 3).

IMPORTANT: For a more thorough presentation of password validation, and of the `fullCheck` method in particular,
please refer to the
link:manual.html[full Manual].

=== Two more configuration examples

Before tackling password generation, let's look at two more examples of `PasswordChecker` configuration.
...
to be composed of upper and lower-case characters, digits and symbols. At least one of there four type of characters
...
upper-case and lowe-case characters and digits (with at least one character in each of these categories).
...
IMPORTANT: Internally `PasswordMaker` uses `ThreadLocalRandom.current()` to generate random numbers used
during password creation. You can specifiy your own source of random numbers if you need to. Please refer
to the link:manual.html[full Manual] for this.

== Conclusion

I hope that this short tutorial gave all the information needed to get started with Password Rodeo. For more
